<template>
    <div>
        <v-dialog v-model="this.$store.state.isFinanceModalOpen" @input="$emit('update:value', $event)" max-width="1000px">
            <v-card>
                                ...
                                <v-card flat>
                                    ...
                                    <v-row>
                                        <v-col md="6" sm="12" lg="6">
                                            ...
                                            <v-select :items="responsiblePersons" item-title="name" variant="outlined"
                                                v-model="finansTahkikYapanKisi"></v-select>
                                        </v-col>
                                        <v-col md="6" sm="12" lg="6">
                                            <h4 class="mb-3">Talep Tamamlanma Tarihi</h4>
                                            <div class="d-flex justify-center">
                                                <VDatePicker v-model="finansTalepTamamlamaTarihi" :popover="false">
                                    ...
                                    <v-row>
                                        <v-col md="12" sm="12" lg="12">
                                            ...
                                            <v-text-field required variant="outlined"
                                                v-model="finansBankaIncelemeSonuclari"></v-text-field>
                                        </v-col>
                                    </v-row>
                                    <v-row>
                                        <v-col md="12" sm="12" lg="12">
                                            <h4 class="mb-3">Açıklama</h4>
                                            <v-text-field required variant="outlined"
                                                v-model="finansAciklama"></v-text-field>
                                        </v-col>
                                    </v-row>
                                </v-card>
                            </v-col>
...
export default {
    ...
    data: () => ({
        finansTahkikYapanKisi: '',
        finansTalepTamamlamaTarihi: ref(new Date()),
        finansBankaIncelemeSonuclari: '',
        finansAciklama: ''
    }),

    methods: {
        ...mapActions(['postFinanceRequestData']),

        onApprove() {
            // Onaylama işlemi
            if (this.$route.name === 'finance' && this.requestData.hukukStatus === null) {
                this.requestData.status = "Hukuk Onayı Bekleniyor"
            }
            
            else if (this.$route.name === 'finance' && this.requestData.hukukStatus !== null) {
        ...
        submitData() {
            const formattedDate = this.finansTalepTamamlamaTarihi.toLocaleDateString("tr-TR", {
                day: "2-digit",
                month: "2-digit",
                year: "numeric"
            });
            const data = {
                ...
                finansTahkikYapanKisi: this.finansTahkikYapanKisi,
                finansTalepTamamlamaTarihi: formattedDate,
                finansBankaIncelemeSonuclari: this.finansBankaIncelemeSonuclari,
                finansAciklama: this.finansAciklama,
                finansStatus: 'Finans Tarafından Onaylandı'
            }
            this.postFinanceRequestData(data);
        },
